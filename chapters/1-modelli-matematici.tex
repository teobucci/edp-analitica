...
\begin{gather*}
    ...
    \\
    \bigcap\limits ^{\infty }_{k=1} C_{k} =\left\{x^{2} +y^{2} \leq 1\right\} \ \text{chiuso!}
\end{gather*}
\begin{figure}[htpb]
    \centering
...
\begin{itemize}
    ...
    \item \textbf{Test di Weierstrass} (condizione sufficiente per la convergenza uniforme):
          ...
          Se $\forall k\geq 1$ esiste $a_{k}$ tale che
          \begin{equation*}
              | f_{k}(x)| \leq a_{k} \ \forall x\in \Omega \ \ \land \ \ \sum\limits ^{\infty }_{k=1} a_{k} < \infty
          \end{equation*}
    ...
    \item \textbf{Scambio serie e limite:}
          ...
          Se $\sum f_{k}$ converge uniformemente in $\Omega $ e $f_{k}$, $k\geq 1$, è continua in $\Omega $, allora $S$ è continua in $\Omega $ e
          \begin{equation*}
    ...
    \item \textbf{Scambio serie e derivata:}
          ...
          Siano $f_{k}$ derivabili in $\Omega$ per ogni $k\geq 1$, rispetto a $x_{j}$ se inoltre:
          \begin{itemize}
