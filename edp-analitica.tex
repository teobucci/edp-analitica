...
\begin{dimostrazione}
	...
	\end{equation*}
	Voglio dimostrare che $\displaystyle g( r)$ è costante e quindi che $\displaystyle g'( r) =0$ per $\displaystyle 0< r< R$. Effettuo un cambio di variabili $\displaystyle \bm{\sigma } '=\frac{\bm{\sigma } -\x}{r}$, ovvero $\displaystyle \bm{\sigma } =\x +r\bm{\sigma } '$, in modo dale che $\displaystyle \bm{\sigma } '\in \partial B_{1}(\zer)$ e
	\begin{equation*}
	...
	\begin{equation*}
	g'( r) =\frac{r}{w_{n}}\int _{B_{1}(\zer)} \Delta u(\x +r\y) d\y\overset{\Delta u=0}{=} 0.
	\end{equation*}
...
\begin{dimostrazione}
	...
	L'integrale su $\displaystyle \partial \Omega $ è già il potenziale di doppio strato, ci manca da far vedere che l'altro tende a $\displaystyle u$, a meno del coefficiente. Calcolo la derivata normale su $\displaystyle \partial B_{\varepsilon }(\x)$ con la regola della catena\begin{equation*}
...
\begin{dimostrazione}
	...
	quindi:
	\begin{equation*}
		\partial _{tt} w_{h}(\x ,t) =\cancel{\frac{c}{4\pi }\int _{\partial B_{1}(\zer)} \nabla h(\x +ct\mathbf{w}) \cdotp \mathbf{w} d\mathbf{w}} -\cancel{\frac{1}{4\pi ct^{2}}\int _{B_{ct}(\x)} \Delta h(\y) d\y} +\frac{\cancel{c}}{4\pi \cancel{c} t}\int _{\partial B_{ct}(\x)} \Delta h(\mathbf{\sigma }) d\mathbf{\sigma } ,
	\end{equation*}
	i primi due termini sono uguali (si vede svolgendo i passaggi di prima per tornare indietro), pertanto:
	\begin{equation*}
		\partial _{tt} w_{h}(\x ,t) =\frac{1}{4\pi t}\int _{\partial B_{ct}(\x)} \Delta h(\mathbf{\sigma }) d\mathbf{\sigma } .
	\end{equation*}
...
Consideriamo la seguente successione\begin{equation*}
v_{n}( x) =\begin{cases}
...
\begin{dimostrazione}
	...
	\begin{itemize}
		...
		\end{equation*}con questa scelta, si ha $\boxed{z\in (\mathrm{Ker} L)^{\perp }}$ e $\Vert z\Vert =1$. Abbiamo\begin{equation*}
		v\in H\ \ \ \ z\in (\mathrm{Ker} L)^{\perp }
		\end{equation*}costruiamo $\boxed{w\in \mathrm{Ker} L}$ che sia combinazione di quei due\begin{equation*}
...
\begin{definition}
	...
	\begin{equation*}
		F\in C^{1}(\overline{\Omega }) \subset L^{2}(\mathcal{\Omega ) \subset D} '( \Omega )
	\end{equation*}
...
\begin{dimostrazione}
	...
	\begin{enumerate}
		\item dimostro la Disuguaglianza di Poincaré (DP) in $\mathcal{D}( \Omega )$,
	...
	\end{gather*}
\end{dimostrazione}
Si noti che in realtà\footnote{e questa è una chicca per l'orale.} per ipotesi serve che $\Omega$ sia limitato almeno in una direzione, ma non per forza tutte. Quando costruiamo il campo ausiliario, è sufficiente scegliere un $\x$ tale che abbia componenti nulle direzioni illimitate e $x_{i}$ nella $i$-esima direzione in cui è limitato. I calcoli si svolgono analogamente. Attenzione che in quel caso la divergenza di $\x$ non è più $n$, ma $1,2, \cdots $ a seconda del caso. Si noti che la stima peggiore dato che non dividiamo per $n$, ma per qualcosa di più piccolo.
\begin{oss}
