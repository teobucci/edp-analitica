...
\end{align*}
$\displaystyle k$ ha legge binomiale di parametri $\displaystyle Bi\left( N,\frac{1}{2}\right)$. % TODO arrivato qui
\begin{itemize}
...
\begin{dimostrazione}
	...
	\end{equation*}
	Voglio dimostrare che $\displaystyle g( r)$ è costante e quindi che $\displaystyle g'( r) =0$ per $\displaystyle 0< r< R$. Effettuo un cambio di variabili $\displaystyle \mathbf{\sigma } '=\frac{\mathbf{\sigma } -\x}{r}$, ovvero $\displaystyle \mathbf{\sigma } =\x +r\mathbf{\sigma } '$, in modo dale che $\displaystyle \mathbf{\sigma } '\in \partial B_{1}(\mathbf{0})$ e
	\begin{equation*}
	d\mathbf{\sigma } =r^{n-1} d\mathbf{\sigma } '
	\end{equation*}
	...
	\begin{figure}[htpb]
		...
		% Text Node
		\draw (201.33,152.57) node [anchor=north west][inner sep=0.75pt]    {$\mathbf{\sigma }$};
		% Text Node
		\draw (279,151.07) node [anchor=north west][inner sep=0.75pt]    {$\mathbf{\sigma } '$};
		% Text Node
		\draw (175.33,84.57) node [anchor=north west][inner sep=0.75pt]    {$d\mathbf{\sigma }$};
		% Text Node
		\draw (212,41.23) node [anchor=north west][inner sep=0.75pt]    {$d\mathbf{\sigma } '$};
		% Text Node
		...
		\mathbf{\sigma } =r\mathbf{\sigma } '\\
		d\mathbf{\sigma } =rd\mathbf{\sigma } '
		\end{array}$};
		\end{tikzpicture}
	\end{figure}
	\FloatBarrier
	...
	\begin{equation*}
	g( r) =\frac{1}{w_{n}\cancel{r^{n-1}}}\int _{\partial B_{1}(\mathbf{0})}\cancel{r^{n-1}} u(\x +r\mathbf{\sigma } ') d\mathbf{\sigma } '=\frac{1}{w_{n}}\int _{\partial B_{1}(\mathbf{0})} u(\x +r\mathbf{\sigma } ') d\mathbf{\sigma } '
	\end{equation*}
	La regione di integrazione ora non dipende da $\displaystyle r$. Calcoliamo la derivata:
	\begin{equation*}
	g'( r) =\frac{1}{w_{n}}\int _{\partial B_{1}(\mathbf{0})}\frac{d}{dr} u(\x +r\mathbf{\sigma } ') d\mathbf{\sigma } '=\frac{1}{w_{n}}\int _{\partial B_{1}(\mathbf{0})} \nabla u(\x +r\mathbf{\sigma } ') \cdotp \mathbf{\sigma } 'd\mathbf{\sigma } '
	\end{equation*}
	Sulla sfera $\displaystyle \partial B_{1}(\mathbf{0})$, il vettore coincide col vettore normale $\displaystyle \mathbf{\sigma } '=\mathbf{\nu }$. Usando il teorema della divergenza e ricordando che $\displaystyle u$ è armonica:
	\begin{equation*}
	...
	\begin{align*}
		...
		g( r) & \xrightarrow{r\rightarrow R}\frac{1}{w_{n} R^{n-1}}\int _{\partial B_{R}(\x)} u(\mathbf{\sigma }) d\mathbf{\sigma }
	\end{align*}
...
\begin{definition}[PDM]
	Più in generale si dice che una funzione $u\in C( \Omega )$ su $\Omega \subset \mathbb{R}^{n}$ soddisfa la proprietà di media superficiale se vale \eqref{eq:lap-prop-media-i} e soddisfa la proprietà di media volumentrica se vale \eqref{eq:lap-prop-media-ii}.
\end{definition}
\begin{theorem}
...
\begin{dimostrazione}
	Partiamo da
	\begin{equation*}
		u(\x) =\frac{1}{w_{n} r^{n-1}}\int _{\partial Br(\x)} u(\mathbf{\sigma }) d\mathbf{\sigma }
	\end{equation*}
	...
	\begin{align*}
		\int _{0}^{R} r^{n-1} u(\x) dr & =\frac{1}{w_{n}}\int _{0}^{R} dr\int _{\partial B_{r}(\x)} u(\mathbf{\sigma }) d\mathbf{\sigma } & \\
...
\label{thm:principio-di-massimo-armoniche}
	[Principio di massimo] Sia $\Omega \subseteq \mathbb{R}^{n}$ e $u\in C(\Omega)$. Se $u$ ha la proprietà della media in $\Omega $, e $u$ assume massimo o minimo in un punto di $\Omega $, allora $u$ è costante. Inoltre, se $\Omega $ è limitato e $u\in C(\overline{\Omega })$ (non costante) allora assume il massimo sul bordo:
	\begin{equation*}
...
\begin{dimostrazione}
	Sia $\displaystyle \mathbf{p}\in \Omega $ punto di minimo
	\begin{equation*}
		m=u(\mathbf{p}) \leqslant u(\y) \ \forall \y\in \Omega 
	\end{equation*}
	Vogliamo dimostrare che $\displaystyle u(x) =m\ \forall x\in \Omega $.
	
	Sia $\displaystyle \mathbf{q}\in \Omega $ arbitrario. Poiché $\displaystyle \Omega $ è connesso, possiamo trovare una sequenza finita di sfere $\displaystyle B(\x_{j}) \subset \subset \Omega,\ j=0,..,N$ tali che
	\begin{itemize}
		\item $\displaystyle \x_{0} =\mathbf{p},\ x_{N} =\mathbf{q}$
	...
	Per assurdo supponiamo esista $\displaystyle \mathbf{z}\in B(\mathbf{p})$ tale che $\displaystyle u(\mathbf{z})  >m$. Sia $\displaystyle B_{r}(\mathbf{z}) \subset B(\mathbf{p})$
	...
	\begin{equation*}
	m=u( \mathbf{p}) =\frac{1}{| B( \mathbf{p})| }\int _{B( \mathbf{p})} u( y) dy=\frac{1}{| B( \mathbf{p})| }\left[\textcolor[rgb]{0.29,0.56,0.89}{\int _{B( \mathbf{p}) \backslash B_{r}( z)} u( y) dy} +\textcolor[rgb]{0.82,0.01,0.11}{\int _{B_{r}( \mathbf{z})} u( y) dy}\right]
	\end{equation*}
	Sul primo integrale $\displaystyle u( y) \geqslant m$.

	Sul secondo integrale posso sfruttare ancora la proprietà della media:
	\begin{equation*}
		\int _{B_{r}(\mathbf{z})} u(y) dy=u(\mathbf{z})| B_{r(\mathbf{z})}|  >m| B_{r}(\mathbf{z})| 
	\end{equation*}
	Ma allora ottengo una contraddizione
	\begin{equation*}
		m >\frac{1}{| B(\mathbf{p})| }\{m| B(\mathbf{p}) \backslash B_{r}(\mathbf{z})| +m| B_{r}(\mathbf{z})| \} =m
	\end{equation*}
	Deduco che $\displaystyle u=m$ in tutto $\displaystyle B(\mathbf{p})$ e quindi anche in $\displaystyle x_{1}$ (poiché $\displaystyle x_{j} \in B(x_{j-1})$) allora posso ripetere il ragionamento che ho fatto trovando $\displaystyle u=m$ in $\displaystyle B(x_{1})$ ed iterando fino all'ultima sfera trovo $\displaystyle u(q) =m$.
\end{dimostrazione}
...
\begin{equation}
	u( x) =-\int _{\Omega } \Phi ( x-y) \Delta u( y) \ dy+\int _{\partial \Omega } \Phi ( x-\sigma ) \partial _{\bm{\nu}} u( \sigma ) \ d\sigma -\int _{\partial \Omega } \partial _{\bm{\nu}} \Phi ( x-\sigma ) u( \sigma ) \ d\sigma 
