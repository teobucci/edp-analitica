...
\textit{Lezione Salsa (fatta da Verzini) (12/04/2021)}
...
\begin{nb}
	...
	\end{equation}
\end{nb}
La cosa positiva è che stiamo studiando un tipo particolare di onde, quelle sferiche, dove $w$ dipende solo dal raggio dal tempo, non dagli angoli, per cui tutti quei termini si annullano.
...
\begin{dimostrazione}
	...
	Ricordando che:
	\begin{equation*}
		\frac{d}{dt}\int _{a}^{f( t)} g( r,t) dr=g( f( t),t) \cdotp f'( t) +\int _{0}^{f( t)} g_{t}( r,t) dr,
	\end{equation*}
	possiamo derivare $e( t)$, ricordando che nel secondo termine l'integrale si può ricomporre:
	\begin{align*}
		\dot{e}( t) & =\frac{1}{2}\int _{\partial B_{c( t_{0} -t)}(\mathbf{x}_{0})}\left[\textcolor[rgb]{0.29,0.56,0.89}{u}\textcolor[rgb]{0.29,0.56,0.89}{_{t}^{2}}\textcolor[rgb]{0.29,0.56,0.89}{+c}\textcolor[rgb]{0.29,0.56,0.89}{^{2}}\textcolor[rgb]{0.29,0.56,0.89}{| }\textcolor[rgb]{0.29,0.56,0.89}{\nabla u}\textcolor[rgb]{0.29,0.56,0.89}{| }\textcolor[rgb]{0.29,0.56,0.89}{^{2}}\right] d\sigma \cdotp \underbrace{\frac{d}{dt} c( t_{0} -t)}_{-c} \\
		            & \ \ \ \ +\frac{1}{\cancel{2}}\int _{B_{c( t_{0} -t)}(\mathbf{x}_{0})}\left[\cancel{2} u_{t} u_{tt} +\cancel{2} c^{2}\textcolor[rgb]{0.82,0.01,0.11}{\nabla u\cdotp \nabla u}\textcolor[rgb]{0.82,0.01,0.11}{_{t}}\right] d\mathbf{x} =( *) .                                                                                                                                                                                                 
	\end{align*}
	Integriamo per parti il termine rosso:
	\begin{equation*}
		\int _{B_{c( t_{0} -t)}(\mathbf{x}_{0})}\textcolor[rgb]{0.82,0.01,0.11}{\nabla u\cdotp \nabla u}\textcolor[rgb]{0.82,0.01,0.11}{_{t}} \ d\mathbf{x} =\int _{\partial B_{c( t_{0} -t)}(\mathbf{x}_{0})} u_{t}\underbrace{u_{\bm{\nu }}}_{\nabla u\cdotp \bm{\nu }} \ d\sigma -\int _{B_{c( t_{0} -t)}(\mathbf{x}_{0})} u_{t}\underbrace{\Delta u}_{\mathrm{div}( \nabla u)} \ d\mathbf{x}
	\end{equation*}
