...
\begin{dimostrazione}
	...
	\end{equation*}
	Voglio dimostrare che $\displaystyle g( r)$ è costante e quindi che $\displaystyle g'( r) =0$ per $\displaystyle 0< r< R$. Effettuo un cambio di variabili $\displaystyle \bm{\sigma } '=\frac{\bm{\sigma } -\x}{r}$, ovvero $\displaystyle \bm{\sigma } =\x +r\bm{\sigma } '$, in modo tale che $\displaystyle \bm{\sigma } '\in \partial B_{1}(\zer)$ e
	\begin{equation*}
...
\begin{theorem}
	...
	\begin{itemize}
		...
		\item Se $\displaystyle q''\geqslant h >0$ e $\displaystyle u_{R}  >u_{L}$ oppure $\displaystyle q''\leqslant -h< 0$ e $\displaystyle u_{R} < u_{L}$
		      ...
		      Dove $r(s)=( q')^{-1}(r(s))$ è la funzione inversa di $\displaystyle q'$
	\end{itemize}
\end{theorem}
...
\begin{equation*}
	...
	\end{cases}
\end{equation*}
Notiamo che $(PCG_{3\varepsilon})$ ha soluzione unica e che $\displaystyle \Gamma $ è a \textbf{simmetria radiale} (invariante per rotazioni in $\displaystyle \mathbb{R}^{3}$). Per l'unicità della soluzione possiamo scrivere:
\begin{equation*}
...
\begin{figure}[htpb]
	...
	% Text Node
	\draw (376,131.4) node [anchor=north west][inner sep=0.75pt]    {$B_{ct}(\mathbf{x})$};
	% Text Node
	\draw (259,140.4) node [anchor=north west][inner sep=0.75pt]    {$\mathbf{x}$};
	% Text Node
	\draw (319,27.4) node [anchor=north west][inner sep=0.75pt]    {$\partial B_{ct}(\mathbf{x} ,x_{3})$};
	% Text Node
